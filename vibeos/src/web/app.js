...
let state = { workspaces:[], memories:[], flows:[], activity:[], settings:{ id:'preferences', theme:'system', motion:'system', initialized:false, installDismissed:false } };
...
function replaceDatabase(records, settings) {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(STORES, 'readwrite');
    transaction.oncomplete = resolve;
    transaction.onerror = () => reject(transaction.error);
    transaction.onabort = () => reject(transaction.error || new Error('The import transaction was aborted.'));
    for (const store of STORES) transaction.objectStore(store).clear();
    for (const store of ['workspaces', 'memories', 'flows', 'activity']) {
      for (const record of records[store]) transaction.objectStore(store).put(record);
    }
  ...
  if (!state.settings.initialized) {
    ...
    if (legacy) {
      try {
        ...
        state.settings = { ...state.settings, id:'preferences', theme:old.mode === 'High Contrast' ? 'contrast' : 'system', initialized:true, migration:'v1' };
    ...
    if (!migrated) {
      ...
      state.settings = { ...state.settings, id:'preferences', initialized:true };
    }
    await put('settings', state.settings);
    await loadState();
  }
...
function normalize(store, item) {
  if (!item || typeof item !== 'object' || Array.isArray(item)) throw new Error(`Invalid ${store} record in import.`);
  const now = Date.now();
  const recordId = typeof item.id === 'string' && UUID_PATTERN.test(item.id) ? item.id : id();
  if (store === 'workspaces') {
    if (!Array.isArray(item.items)) throw new Error('An imported workspace has invalid actions.');
    return { id:recordId, title:typeof item.title === 'string' ? item.title : 'Untitled workspace', kind:typeof item.kind === 'string' ? item.kind : 'General', status:item.status === 'done' ? 'done' : 'active', summary:typeof item.summary === 'string' ? item.summary : '', items:item.items.map(action => typeof action === 'string' ? {id:id(),text:action,done:false} : normalizeAction(action)), createdAt:Number.isFinite(item.createdAt) ? item.createdAt : now, updatedAt:now };
  }
  if (store === 'memories') return { id:recordId, type:typeof item.type === 'string' ? item.type : 'note', title:typeof item.title === 'string' ? item.title : 'Untitled memory', body:typeof item.body === 'string' ? item.body : '', pinned:!!item.pinned, createdAt:Number.isFinite(item.createdAt) ? item.createdAt : now, updatedAt:now };
  if (store === 'flows') return { id:recordId, name:typeof item.name === 'string' ? item.name : 'Untitled flow', text:typeof item.text === 'string' ? item.text : '', enabled:item.enabled !== false, createdAt:Number.isFinite(item.createdAt) ? item.createdAt : now, updatedAt:now };
  return { id:recordId, text:typeof item.text === 'string' ? item.text : 'Imported activity', time:typeof item.time === 'string' ? item.time : new Date().toISOString() };
}
function normalizeAction(item) {
  if (!item || typeof item !== 'object' || Array.isArray(item)) throw new Error('An imported workspace action is invalid.');
  return { id:typeof item.id === 'string' && UUID_PATTERN.test(item.id) ? item.id : id(), text:typeof item.text === 'string' ? item.text : '', done:!!item.done };
}
async function saveRecord(store, record) { await put(store, record); state[store] = await readAll(store); render(); }
async function activity(text) { const record = { id:id(), text, time:new Date().toISOString() }; await put('activity', record); const all = (await readAll('activity')).sort((a,b) => new Date(b.time) - new Date(a.time)); await Promise.all(all.slice(100).map(item => remove('activity', item.id))); state.activity = all.slice(0, 100); }
...
function workspaceDialog(workspace) {
  ...
  $('#shareWorkspace')?.addEventListener('click', () => { const text=`${draft.title}\n\n${draft.summary}\n\n${draft.items.map(item=>`${item.done?'✓':'○'} ${item.text}`).join('\n')}`; if (nativeBridge()) callNative('shareText', text); else if (navigator.share) navigator.share({title:draft.title,text}).catch(error=>{ if (error.name !== 'AbortError') toast('Sharing is unavailable in this browser.'); }); else toast('Sharing is unavailable in this browser.'); });
}
function bindMemoryControls() {
  ...
  document.querySelectorAll('[data-delete-memory]').forEach(button=>button.onclick=async()=>{const item=state.memories.find(memory=>memory.id===button.dataset.deleteMemory);if (!item || !confirm(`Delete "${item.title}" from local memory?`)) return;await remove('memories',item.id);state.memories=await readAll('memories');await activity(`Deleted memory: ${item.title}`);render();});
}
function bind() {
  ...
  $('#privacyShortcut').onclick = () => dialog('Privacy Center','<p>VibeOS stores its core data only in this app or browser profile. It has no account, cloud sync, analytics, or live connectors.</p><p>Export and share actions are initiated by you. In the Android APK, notification context and the foreground app package are optional system-granted features stored locally by the app.</p>');
  ...
  $('#memorySearch')?.addEventListener('input',event=>{ $('#memoryResults').innerHTML=memoryList(state.memories.filter(item=>`${item.title} ${item.body}`.toLowerCase().includes(event.target.value.toLowerCase()))); bindMemoryControls(); });
  bindMemoryControls();
...
async function importData(event) { const file=event.target.files[0];if(!file)return;try { const parsed=JSON.parse(await file.text());if(parsed.schemaVersion!==2 || !Array.isArray(parsed.workspaces)||!Array.isArray(parsed.memories)||!Array.isArray(parsed.flows)||('activity' in parsed&&!Array.isArray(parsed.activity))) throw new Error('This is not a valid VibeOS version 2 export.');const records={workspaces:parsed.workspaces.map(item=>normalize('workspaces',item)),memories:parsed.memories.map(item=>normalize('memories',item)),flows:parsed.flows.map(item=>normalize('flows',item)),activity:(parsed.activity||[]).map(item=>normalize('activity',item))};const settings={id:'preferences',theme:['system','light','dark','contrast'].includes(parsed.settings?.theme)?parsed.settings.theme:'system',motion:['system','reduce'].includes(parsed.settings?.motion)?parsed.settings.motion:'system',initialized:true};if(!confirm('Replace local VibeOS data with this export?'))return;await replaceDatabase(records,settings);localStorage.removeItem(LEGACY_KEY);await loadState();await activity('Imported VibeOS data.');render();toast('Local data imported.'); } catch(error) { console.error('Import failed',error);toast(error.message || 'The selected file could not be imported.'); } finally { event.target.value=''; } }
async function resetData() { if(!confirm('Reset all VibeOS data stored on this device? This cannot be undone.'))return;await resetDb();localStorage.removeItem(LEGACY_KEY);state={workspaces:[],memories:[],flows:[],activity:[],settings:{id:'preferences',theme:state.settings.theme,motion:state.settings.motion,initialized:false}};await loadState();render();toast('Local data was reset.'); }
